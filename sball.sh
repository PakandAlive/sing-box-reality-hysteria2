...
hint() { echo -e "\033[35m\033[01m$*\033[0m"; }   # 紫色
...
check_dependencies() {
  # 如果是 Alpine，��升级 wget ，安装 systemctl-py 版
  if [ "$SYSTEM" = 'Alpine' ]; then
...
# 传参处理2: 去掉 sudo cloudflared service install ，以��便用户输入 Token 并能正确读取真正的以 ey 开头的 Value
